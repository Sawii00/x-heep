...
enum CHANNEL{A=0x0, B=0x1, C=0x2, D=0x3, E=0x4, F=0x5, G=0x6, H=0x7, ALL=0xF};

static inline __attribute__((always_inline)) uint16_t adc_2_dac_scale(uint16_t val)
{
	float temp = ((float)val * 33 ) / 25;
...
void set_voltage(spi_host_t *SPI, enum CHANNEL ch, uint32_t val)
{
    //spi_wait_for_ready(SPI);
  	uint32_t cmd = 0x03000000 | (ch << 20) | ((val & 0xFFF) << 8);
		// TODO: check if buildin is faster or not than previous function (which gets inlined anyway)
...
volatile uint16_t vals[N_SAMPLES];
volatile uint16_t results[N_SAMPLES];

void write_to_flash(spi_host_t *SPI, dma_t *DMA, uint16_t *data, uint32_t count)
{
    // Write enable
    ...
    dma_set_cnt_start(DMA, (uint32_t) count*sizeof(*data)); // Size of data received by SPI
...
int main(int argc, char *argv[])
{
    ...
    prepare_sin_table(sin_lookup, N_SAMPLES, 1000, 100000);


		for(uint32_t i = 0; i < N_SAMPLES; ++i)
		{
        set_voltage(&spi_host_dac, ALL, sin_lookup[i]);
				for(volatile uint32_t k = 0; k < 10; ++k);
				results[i] = get_voltage(&spi_host_adc);
				//for(volatile uint32_t k = 0; k < 10; ++k);
    }

		write_to_flash(&spi_host_flash, &dma, results, N_SAMPLES);
